...
class NIHCXRDataModule(pl.LightningDataModule):
    def __init__(self, data_dir, resolution, binary=False, batch_size=16):
    ...
    def setup(self, stage: Optional[str] = None):
        if stage == "fit" or stage is None:
            self.dataset_train = NIHCXRDataset(
                self.data_dir,
                split="train",
                binary=self.binary,
                resolution=self.resolution,
            )
            self.dataset_val = NIHCXRDataset(
                self.data_dir,
                split="val",
                binary=self.binary,
                resolution=self.resolution,
        ...
        if stage == "test" or stage is None:
            self.dataset_test = NIHCXRDataset(
                self.data_dir,
                split="test",
                binary=self.binary,
                resolution=self.resolution,
...
class NIHCXRDataset(Dataset):
    def __init__(self, data_dir, split, resolution, binary=False):
        ...
        self.labels_df = pd.read_csv(
            os.path.join(data_dir, "labels.csv"), index_col="image_id"
        ).loc[:, conditions]
        self.binary = binary
        self.resolution = resolution
...
def make_labels_file(data_dir):
    ...
    for i in range(len(source_df)):
        ...
        target_datapoint = {
            "image_id": source_datapoint["Image Index"],
...
def make_split_dirs(image_dir, split_list_dir, val_size=0.2):
    ...
    train_filenames = [
        ...
        if filename.split("_")[0] in train_subject_ids
    ]
    val_filenames = [
        filename
        for filename in train_val_filenames
        if filename.split("_")[0] in val_subject_ids
    ]
