...
class LeftRightDataset(Dataset):
    ...
    def __len__(self):
        return len(self.split_file) * 2

    def __getitem__(self, item):
        filename = self.split_file.iloc[item // 2, 0]
        ...
        image_path = os.path.join(self.data_dir, "512p/homebrew", filename)
        image = Image.open(image_path).convert("RGB")
        if self.grayscale:
            image = image.mean(dim=0, keepdim=True)
        image = np.array(image)
        image = Image.fromarray(image)
        image.save(f"{id}-before-masking.png")

        mask_path = os.path.join(self.data_dir, "512p_segment-mask/homebrew", filename)
        whole_mask = np.array(Image.open(mask_path))
        mask = whole_mask == mask_val
        image = image * mask[..., np.newaxis]
        image = Image.fromarray(image)
        image.save(f"{id}-after-masking.png")
        image = self.transform(image)

        if self.trend:
            image_3hr_path = os.path.join(self.data_dir, "512p/homebrew", filename)
            mask_3hr_path = os.path.join(
                self.data_dir, "512p_segment-mask/homebrew", filename
            )
    ...
    def get_original_image(self, item):
        ...
        image_path = os.path.join(self.data_dir, "512p/homebrew", image_file)
        image = Image.open(image_path).convert("RGB")
...
def test_dataloader(data_dir):
    ...
    # ids, labels_left, labels_right = dataset.get_ids_and_labels()
