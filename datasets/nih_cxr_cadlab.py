...
class NIHCXRCadLabDataModule(pl.LightningDataModule):
    ...
    def __init__(self, data_dir, resolution=224, batch_size=16):
    ...
    def setup(self, stage: Optional[str] = None):
        if stage == "fit" or stage is None:
            self.dataset_train = NIHCXRCadLabDataset(self.data_dir, split='train', resolution=self.resolution)
            self.dataset_val = NIHCXRCadLabDataset(self.data_dir, split='val', resolution=self.resolution)
        if stage == "test" or stage is None:
            self.dataset_test = NIHCXRCadLabDataset(self.data_dir, split='test', resolution=self.resolution)
...
class NIHCXRCadLabDataset(Dataset):
    ...
    def __init__(self, data_dir, split, resolution=224):
